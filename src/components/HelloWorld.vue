<template>
  <div class="hello">
    <h1>Välkommen till vue demo</h1>
    <h1> {{ exempel }}</h1>
    <p>{{text}}</p>

    <button v-on:click ="handleClick">+</button>
     <button v-on:click ="minusClick">-</button>

      <button v-on:click="resetclick">reset</button>

       <button class="show" v-show="showButton" v-on:click="veiwText">-</button>

       <section> 
       <h2>övnin 3</h2>
       <button v-on:click ="hideText">{{textIsVisible ? 'göm' : 'visa'}}  text</button>
       <span v-show="textIsVisible">now u see me</span>
       <span v-show="!textIsVisible">now u dont</span>
       </section> 
       
       <section>
       <h2> övning 5</h2>
       <div class="carousel">
       <button v-on:click="previouseIndex"> &lt;</button>
       <img :src="images[currentIndex]"/>
       <button v-on:click="nextIndex">&gt;</button>
        </div>
       </section>
  </div> 
</template>
